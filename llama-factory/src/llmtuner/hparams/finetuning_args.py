...
class FinetuningArguments(
    FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments
):
    r"""
    ...
    ema_decay: str = field(
        default=None,
        metadata={"help": "Decay for EMA of the online Model. Accept single decay value and multiple ones, splitted by '_'. E.g. '0.75_0.85'"},
    )
    ema_update_freq: int = field(
        default=1,
        metadata={"help": "Frequency of updating EMA."},
    )
    ema_on_cpu: bool = field(
        default=True,
        metadata={"help": "Whether to compute EMA on CPUs."},
    )
    ema_local_init_model_name: str = field(
        default=None,
        metadata={"help": "Load a local EMA model for initialization."},
    )
